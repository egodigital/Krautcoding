...
//const gtfs = new GTFS(); // TODO needed?
var heart_beat = new HeartBeat(timeRef, trackerConfig.simulation);
//new TimeTable(timeRef, panelsRef, gtfs, panelConfig, googleMapsClient); // TODO
...
if (trackerConfig.simulation) {
  ...
  */
  ...
  const {PathGenerator} = require('./path_generator.js');
  const {BusSimulator} = require('./bus_simulator.js');

  paths = []; // this must be passed as reference, does it?
  new PathGenerator(heart_beat.now, busLocationsRef, panelConfig, paths);
  // TODO do not hand gtfs?, substitue gtfs with something which knows all route ids??
  new BusSimulator(timeRef, busLocationsRef, paths);
}
