...
exports.BusSimulator = class {
  constructor(timeRef, busLocationsRef, generatedPaths) {
  ...
  busAdvance(now) {
    ...
    buses.forEach(bus => {
      ...
      busLocations[`Trip_${bus.trip.trip_id}`] = {
        ...
        route_color: bus.trip.color,
        route_name: `Trip_${bus.trip.trip_id}`,
  ...
  getBusesActiveAt(time) {
    const buses = [];
    this.paths.forEach(bus => { // avoid error on empty
